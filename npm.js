...
// Behind the scenes it works as a node package manager.But actually it does not stands for node package manager.
...
The terms "package" and "library" are often used interchangeably in the context of software development, especially when discussing package managers like NPM. However, there are subtle differences between the two concepts.
...
Cross-origin refers to the practice of making requests to a domain different from the one that the currently loaded web page is on.
In the context of web development, "origin" refers to the combination of the protocol (http or https), domain, and port of a URL. 
Cross-origin requests happen when these components differ between the requesting page and the resource being requested.

Cross-Origin Resource Sharing (CORS): A security feature implemented by web browsers to restrict cross-origin requests and prevent unauthorized access to resources.
...
// https://browserslist.dev/?q=bGFzdCAxIHZlcnNpb24%3D